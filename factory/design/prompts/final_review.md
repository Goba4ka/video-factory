...
- passed QCReport, immutable QCEvidenceBundle, and contact sheet, all bound to the
  same render SHA-256;
...
If any item is missing, any checksum differs, QC/evidence is not passed, or rights
need review, return `review_blocked`. Otherwise return
`awaiting_human_decision`. Never infer consent
